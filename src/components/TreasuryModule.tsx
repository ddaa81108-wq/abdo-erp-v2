...
import { Landmark, ArrowUpRight, ArrowDownLeft, Wallet, Sparkles, Plus, Minus, Trash2, FileText, UserCircle, RotateCcw, ShieldAlert } from 'lucide-react';
...
export default function TreasuryModule({ state, onUpdateState, onOpenExporter }: TreasuryModuleProps) {
  ...
  const totalPositiveDebts = totalCustomerDebts + totalCompanyDebts + totalMerchantDebts;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        Object.values(parsed).forEach((merch: any) => {
          const prev = merch.previousBalance || 0;
          const rowsRes = (merch.rows || []).reduce((s: number, r: any) => s + (r.result || 0), 0);
          const rowsPaid = (merch.rows || []).reduce((s: number, r: any) => s + (r.paid || 0), 0);
  ...
  const validTreasuryTxs = (state.treasuryTransactions || []).filter(tx => !tx.isDeleted);
  ...
  const handleDeposit = (e: React.FormEvent) => {
    ...
    const newTx: TreasuryTransaction = {
      ...
      description: `[المودع: ${depositActor.trim()}] ${depositNote.trim()}`.trim(),
  ...
  const handleWithdraw = (e: React.FormEvent) => {
    ...
    const newTx: TreasuryTransaction = {
      ...
      description: `[الساحب: ${withdrawActor.trim()}] ${withdrawNote.trim()}`.trim(),
      date: new Date().toISOString(),
      createdAt: new Date().toISOString()
    };
  ...
  const executeResetTreasury = () => {
    if (resetConfirmWord === 'تأكيد') {
      const updatedTxs = (state.treasuryTransactions || []).map(tx => ({ ...tx, isDeleted: true }));
      onUpdateState({
        ...state,
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4">
              <span className="text-white font-extrabold text-sm tracking-wide">إجمالي إيجابيات الخزينة</span>
            ...
            <div className="mt-auto">
              <div className="text-3xl font-black text-white drop-shadow-md">
                {grandTotalPositives.toLocaleString()} <span className="text-sm font-bold opacity-70">د.ل</span>
              </div>
        ...
        <div className="bg-emerald-600 border border-emerald-500 rounded-2xl p-5 shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4">
              <span className="text-emerald-50 font-extrabold text-sm tracking-wide">الرصيد النشط (الكاش الفعلي)</span>
              <div className="bg-white/10 p-2 rounded-xl backdrop-blur-md">
            ...
            <div className="mt-auto">
              <div className="text-3xl font-black text-white drop-shadow-md">
                {activeCash.toLocaleString()} <span className="text-sm font-bold opacity-70">د.ل</span>
              </div>
              <div className="text-[10px] text-emerald-200 mt-2 font-medium">السيولة النقدية المُدارة فعلياً باليد الآن</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-rose-600 border border-rose-500 rounded-2xl p-5 shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4">
              <span className="text-rose-50 font-extrabold text-sm tracking-wide">الالتزامات (علينا)</span>
              <div className="bg-white/10 p-2 rounded-xl backdrop-blur-md">
            ...
            <div className="mt-auto">
              <div className="text-3xl font-black text-white drop-shadow-md">
                {totalLiabilities.toLocaleString()} <span className="text-sm font-bold opacity-70">د.ل</span>
              </div>
              <div className="text-[10px] text-rose-100 mt-2 font-medium">مجموع (الأمانات + المشتريات) المتبقية</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-indigo-600 border border-indigo-500 rounded-2xl p-5 shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4">
              <span className="text-indigo-50 font-extrabold text-sm tracking-wide">صافي الخزينة المعياري</span>
              <div className="bg-white/10 p-2 rounded-xl backdrop-blur-md">
            ...
            <div className="mt-auto">
              <div className="text-3xl font-black text-white drop-shadow-md">
                {netTotal.toLocaleString()} <span className="text-sm font-bold opacity-70">د.ل</span>
              </div>
              <div className="text-[10px] text-indigo-100 mt-2 font-medium">إجمالي الإيجابيات (الكارت الأول) - الالتزامات (الكارت الثالث)</div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="bg-white border text-right border-emerald-100 rounded-xl p-3 shadow-sm">
           ...
           <form onSubmit={handleDeposit} className="space-y-2">
             <div className="grid grid-cols-2 gap-2">
               <div>
                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">المبلغ المودع</label>
                  <input
                    type="number"
                    required
                    value={depositAmount}
               ...
               <div>
                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">اسم المودع</label>
             ...
             <div>
                <label className="block text-[10px] font-bold text-slate-600 mb-0.5">البيان (اختياري)</label>
                <input
                  type="text"
                  placeholder="ملاحظات..."
                  value={depositNote}
             ...
             <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-1.5 rounded-lg transition text-[11px]">
               تأكيد الإدخال
             </button>
           </form>
        </div>
        ...
        <div className="bg-white border text-right border-rose-100 rounded-xl p-3 shadow-sm">
           ...
           <form onSubmit={handleWithdraw} className="space-y-2">
             <div className="grid grid-cols-2 gap-2">
               <div>
                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">المبلغ المسحوب</label>
                  <input
                    type="number"
                    required
               ...
               <div>
                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">اسم المستلم أو الساحب</label>
                  <div className="relative">
                    <UserCircle className="w-3 h-3 absolute top-1/2 right-2 -translate-y-1/2 text-slate-400" />
                    <input
                      type="text"
             ...
             <div>
                <label className="block text-[10px] font-bold text-slate-600 mb-0.5">التفاصيل والبيان</label>
                <input
                  type="text"
                  required
             ...
             <button type="submit" className="w-full bg-rose-600 hover:bg-rose-700 text-white font-bold py-1.5 rounded-lg transition text-[11px]">
               تأكيد وخصم
             </button>
           </form>
        </div>
         ...
         <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 items-start">
            ...
            <div>
              <div className="flex items-center justify-between bg-emerald-50 px-3 py-2 border border-emerald-100 rounded-t-lg">
                <span className="text-emerald-800 font-extrabold text-xs">إيداعات بالخزينة</span>
                <span className="text-emerald-600 font-bold text-[10px] bg-emerald-100 px-2 py-0.5 rounded-md">
                  {validTreasuryTxs.filter(tx => tx.type === 'in').length} عمليات
                </span>
              </div>
              <div className="w-full border-x border-b border-emerald-100 rounded-b-lg overflow-x-auto">
                <table className="w-full text-right whitespace-nowrap">
                  <thead className="bg-emerald-50/50">
                    <tr>
                      <th className="px-2 py-1.5 text-emerald-700 font-black border-b border-emerald-100 text-[9px] w-8 text-center">ت</th>
                      <th className="px-2 py-1.5 text-emerald-700 font-bold border-b border-emerald-100 text-[10px]">الاسم</th>
                      <th className="px-2 py-1.5 text-emerald-700 font-bold border-b border-emerald-100 text-[10px]">المبلغ</th>
                      <th className="px-2 py-1.5 text-emerald-700 font-bold border-b border-emerald-100 text-[10px]">البيان</th>
                      <th className="px-2 py-1.5 text-emerald-700 font-bold border-b border-emerald-100 text-[10px]">التاريخ واليوم</th>
                      <th className="px-2 py-1.5 text-emerald-700 font-bold border-b border-emerald-100 text-[10px] w-8"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-emerald-50">
                    {validTreasuryTxs.filter(tx => tx.type === 'in').map((tx, idx, arr) => {
                      const match = tx.description.match(/\[(.*?)\](.*)/);
                      const actorName = match ? match[1].replace('المودع:', '').replace('الساحب:', '').trim() : 'متفرقات';
                      ...
                      return (
                        <tr key={tx.id} className="hover:bg-emerald-100 transition even:bg-emerald-50 odd:bg-white border-b border-emerald-100">
                          <td className="px-2 py-1 text-emerald-600/70 font-black text-[9px] text-center border-l border-emerald-50 bg-emerald-50/20">{seqNum}</td>
                          <td className="px-2 py-1 text-slate-700 font-bold text-[10px] max-w-[80px] truncate" title={actorName}>{actorName}</td>
                          <td className="px-2 py-1 text-emerald-700 font-black text-[10px] font-mono">+{tx.amount.toLocaleString()}</td>
                          <td className="px-2 py-1 text-slate-500 font-medium text-[9px] max-w-[100px] truncate" title={noteStr}>{noteStr || '-'}</td>
                          <td className="px-2 py-1 text-slate-500 font-mono text-[9px]">
                            {d.toLocaleDateString('ar-EG', { weekday: 'short' })} {d.toLocaleDateString('en-GB')}
                          </td>
                          <td className="px-2 py-1 text-center">
                            <button onClick={() => setDeleteConfirmId(tx.id)} className="text-emerald-400 hover:text-rose-500 transition cursor-pointer">
                    ...
                    {validTreasuryTxs.filter(tx => tx.type === 'in').length === 0 && (
                      <tr><td colSpan={6} className="px-2 py-3 text-center text-[10px] text-emerald-500/60 font-bold">لا يوجد إيداعات حتى الآن</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between bg-rose-50 px-3 py-2 border border-rose-100 rounded-t-lg">
                <span className="text-rose-800 font-extrabold text-xs">سحوبات من الخزينة</span>
                <span className="text-rose-600 font-bold text-[10px] bg-rose-100 px-2 py-0.5 rounded-md">
                  {validTreasuryTxs.filter(tx => tx.type === 'out').length} عمليات
                </span>
              </div>
              <div className="w-full border-x border-b border-rose-100 rounded-b-lg overflow-x-auto">
                <table className="w-full text-right whitespace-nowrap">
                  <thead className="bg-rose-50/50">
                    <tr>
                      <th className="px-2 py-1.5 text-rose-700 font-black border-b border-rose-100 text-[9px] w-8 text-center">ت</th>
                      <th className="px-2 py-1.5 text-rose-700 font-bold border-b border-rose-100 text-[10px]">الاسم</th>
                      <th className="px-2 py-1.5 text-rose-700 font-bold border-b border-rose-100 text-[10px]">المبلغ</th>
                      <th className="px-2 py-1.5 text-rose-700 font-bold border-b border-rose-100 text-[10px]">البيان</th>
                      <th className="px-2 py-1.5 text-rose-700 font-bold border-b border-rose-100 text-[10px]">التاريخ واليوم</th>
                      <th className="px-2 py-1.5 text-rose-700 font-bold border-b border-rose-100 text-[10px] w-8"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-rose-50">
                    {validTreasuryTxs.filter(tx => tx.type === 'out').map((tx, idx, arr) => {
                      const match = tx.description.match(/\[(.*?)\](.*)/);
                      const actorName = match ? match[1].replace('المودع:', '').replace('الساحب:', '').trim() : 'متفرقات';
                      const noteStr = match ? match[2].trim() : tx.description;
                      const d = new Date(tx.date);
                      const seqNum = arr.length - idx; // new descending seq
                      return (
                        <tr key={tx.id} className="hover:bg-rose-100 transition even:bg-rose-50 odd:bg-white border-b border-rose-100">
                          <td className="px-2 py-1 text-rose-600/70 font-black text-[9px] text-center border-l border-rose-50 bg-rose-50/20">{seqNum}</td>
                          <td className="px-2 py-1 text-slate-700 font-bold text-[10px] max-w-[80px] truncate" title={actorName}>{actorName}</td>
                          <td className="px-2 py-1 text-rose-700 font-black text-[10px] font-mono">-{tx.amount.toLocaleString()}</td>
                          <td className="px-2 py-1 text-slate-500 font-medium text-[9px] max-w-[100px] truncate" title={noteStr}>{noteStr || '-'}</td>
                          <td className="px-2 py-1 text-slate-500 font-mono text-[9px]">
                            {d.toLocaleDateString('ar-EG', { weekday: 'short' })} {d.toLocaleDateString('en-GB')}
                          </td>
                          <td className="px-2 py-1 text-center">
                            <button onClick={() => setDeleteConfirmId(tx.id)} className="text-rose-400 hover:text-rose-600 transition cursor-pointer">
                              <Trash2 className="w-3 h-3 mx-auto" />
                            </button>
                          </td>
                        </tr>
                    ...
                    {validTreasuryTxs.filter(tx => tx.type === 'out').length === 0 && (
                      <tr><td colSpan={6} className="px-2 py-3 text-center text-[10px] text-rose-500/60 font-bold">لا يوجد سحوبات حتى الآن</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 shadow-2xl w-full max-w-sm text-center" dir="rtl">
            <div className="mx-auto w-12 h-12 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center mb-4">
              <Trash2 className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-bold text-slate-800 mb-2">تأكيد حذف المعاملة</h3>
            <p className="text-sm text-slate-500 mb-6">هل أنت متأكد من حذف هذه المعاملة النشطة والخزينة ستقوم بتعديل الرصيد؟</p>
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white border-2 border-rose-500 rounded-3xl p-6 shadow-2xl w-full max-w-md text-right border" dir="rtl">
            ...
            <p className="text-sm font-semibold text-slate-600 mb-4 leading-relaxed">
              هذا الإجراء سيقوم بحذف كافة الحركات النقدية (الإيداع والسحب) من الخزينة المركزية بشكل، وسيتم إعادة الرصيد النشط إلى صفر د.ل. 
            </p>
            
            <div className="bg-rose-50 border border-rose-200 p-4 rounded-xl mb-6">
              <p className="text-xs font-bold text-rose-800 mb-2">للتأكيد، يرجى كتابة كلمة "تأكيد" في المربع أدناه:</p>
              <input 
                type="text"
